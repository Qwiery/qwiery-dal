...
import {DefaultAdapter} from "./defaultStore/defaultAdapter.js";
 ...
 * Gateway to graphs.
 * */
import {RandomGraph} from "@orbifold/graphs";

export class Qwiery extends EventEmitter {
    /**
    ...
    storageApis:any[] = [];
    storageInitErrors:string[] = [];
    static adapters = [DefaultAdapter];
    /**
     * Dictionary of adapterId-adapterClass.
     * @type {{ [key: string]: any; }}
     */
    static adapterDic:{ [key: string]: any; } = {
        memory: DefaultAdapter,
    };
     ...
     */
    static get info() {
        return `Qwiery v${pkg.version} by ${pkg.author.name}. ${pkg.description}`;
    }
    ...
    async randomGraph(name = "Erdos", ...args) {
        const g = RandomGraph.create(<any>name, ...args);
